  ...
  -------------------------------------------------------------------------------------*/
    var filterProducts = function () {
        ...
        $('input[name="availability"]').on("change", function () {
            filters.availability = $(this).attr("id") === "inStock" ? "In Stock" : "Out Stock";
            applyFilters();
            updateMetaFilter();
            updatePagination();
        });
        $('input[name="size"]').on("change", function () {
            ...
            filters.size = label.find(".size-text").text().trim();
            applyFilters();
            updateMetaFilter();
            updatePagination();
        });
        $('input[name="color"]').on("change", function () {
            ...
            filters.color = label.data("color-text");
            applyFilters();
            updateMetaFilter();
            updatePagination();
        });
        ...
        $('input[name="category"]').on("change", function () {
            ...
            const label = $(`label[for="${cateId}"]`);
            filters.category = label.find(".cate-text").text().trim();
            applyFilters();
            updateMetaFilter();
            updatePagination();
        });
        ...
        function updatePagination() {
            if ($(".meta-filter-shop").hasClass("active") == true) {
                ...
                $("#listLayout .not-filter").css("display", "none");
                $("#gridLayout .not-filter").css("display", "none");
            }
        }
        function updateMetaFilter() {
            ...
            if (filters.brand) {
                appliedFilters.append(
                    `<span class="filter-tag remove-tag " data-filter="brand"><span class="icon icon-Close"></span> Brand: ${filters.brand}</span>`
                );
            }
            if (filters.category) {
                appliedFilters.append(
                    `<span class="filter-tag remove-tag " data-filter="category"><span class="icon icon-Close"></span> Category: ${filters.category}</span>`
                );
            }
        ...
        function applyFilters() {
            ...
            const allProduct = $(".wrapper-shop.tf-grid-layout .card-product").length;
            const allProductList = $(".wrapper-shop.tf-list-layout .card-product").length;

            $("#product-count-grid").html(
                `Showing <span class="count">${visibleProductCountGrid}</span> of ${allProduct} ${visibleProductCountGrid === 1 ? "products" : "products"}`
            );

            $("#product-count-list").html(
                `Showing <span class="count">${visibleProductCountList}</span> of ${allProductList} ${visibleProductCountList === 1 ? "products" : "products"}`
            );
        }
    };
    ...
    var swLayoutShop = function () {
        ...
        function setGridLayout(layoutClass) {
            ...
            if (layoutClass === "tf-col-4") {
                $("#gridLayout .not-filter").css("display", "block");
            } else {
                $("#gridLayout .not-filter").hide();
            }
        }
        ...
        $(document).ready(function () {
            ...
            $("#product-count-grid").toggle(!isListActive);
            $("#product-count-list").toggle(isListActive);
        });
        ...
        $(".tf-view-layout-switch").on("click", function () {
            ...
            $("#product-count-grid").toggle(layout !== "list");
            $("#product-count-list").toggle(layout === "list");
